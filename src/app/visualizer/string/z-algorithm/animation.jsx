...
export default function Animation() {
  const [text, setText] = useState("aabcaabxaaaz");
  const [pattern, setPattern] = useState("aabx");
  const [index, setIndex] = useState(-1);

  const runDemo = () => {
    // Demonstration using JavaScript's built-in search.
    // The actual Z Algorithm implementation is shown in the code section.
          ...
          {index === -1 ? (
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">
                The Z Algorithm builds a Z-array for the combined string
                <span className="font-semibold"> "{pattern + "$" + text}" </span>
                and identifies the pattern by finding Z-values equal to the
                pattern length.
              </p>
            </div>
          )}
